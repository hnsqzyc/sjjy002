...
class JjySpider(scrapy.Spider):
    ...
    def start_requests(self):
        while 1: # 可能造成重复
            result = self.sjjy.find({'status': 0}, {'_id': 0, 'realUid': 1}).limit(100)
            if result.count():
                for res in result:
                    meta = {}
                    ...
                    result = self.sjjy.update({'realUid':  meta['realUid']}, {'$set': {'status': 1}})  #  拿到以后不用等下载图片数量及链接,马上把状态修改了
